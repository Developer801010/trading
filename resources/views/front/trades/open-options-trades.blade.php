    ...
    <div class="main-feed">
        <div class="container-lg">
            <div class="d-flex gap-3 flex-wrap justify-content-between mb-4">
                <h1 class="title">Open Options Trades</h1>
                <div class="search-input">
                    <form action="{{ route('front.open-options-trades') }}" method="GET" class="mainFeedSearch">
                        ...
                        <tbody>
                            @if(isset($trades) && (count($trades) > 0))
                                ...
                                <tr  @if($trade->tradeDetail !== null && $trade->tradeDetail->count()) class="view" @endif>
                                    <td class="text-uppercase" data-trade-id="{{ $trade->id }}">
                                    ...
                                    <td>@if($trade->trade_direction == 'buy') Long @else Short @endif</td>
                                    <td>{{\Carbon\Carbon::parse($trade->entry_date)->format('m/d/Y')}}</td>
                                    <td>${{ $trade->current_price }} {{-- ({{ rtrim(rtrim(number_format($trade->position_size, 1), '0'), '.') }}%) --}}</td>
                                ...
                                @if($trade->tradeDetail !== null && $trade->tradeDetail->count())
                                    @foreach($trade->tradeDetail as $childTrade)
                                        ...
                                        <tr class="fold">
                                            ...
                                            <td>
												${{ $childTrade->entry_price }} {{-- ({{ rtrim(rtrim(number_format($childTrade->position_size, 1), '0'), '.') }}%) --}}
                                            </td>
